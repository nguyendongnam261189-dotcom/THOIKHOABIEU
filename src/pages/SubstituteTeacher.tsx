          ...
          <table className="min-w-full divide-y divide-gray-200 text-xs text-center border-collapse">
            <thead className="bg-gray-100">
              <tr>
                ...
                {days.map(day => (
                  <th key={day} className="px-1 py-2 font-medium text-gray-500 border-r border-gray-200 last:border-0 min-w-[40px]">
                    Thứ {day}
                  </th>
                ))}
              </tr>
  ...
  const renderAbsentTeacherSchedule = (teacherName: string) => {
    ...
    return (
      <div key={teacherName} className="mb-6 bg-white p-4 md:p-6 rounded-xl shadow-sm border border-gray-200">
        ...
        <p className="text-xs md:text-sm text-gray-500 mb-4 italic">Chạm vào tiết để phân công.</p>
        <div className="overflow-x-auto rounded-xl border border-gray-200">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-2 py-2 md:px-4 md:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-r whitespace-nowrap">Tiết \ Thứ</th>
                {days.map(day => (
                  <th key={day} className="px-2 py-2 md:px-4 md:py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider border-r">
                    T{day}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {periods.map(period => (
                <tr key={period} className={period === 5 ? 'border-b-4 border-gray-300' : ''}>
                  <td className="px-2 py-2 md:px-4 md:py-3 whitespace-nowrap text-xs md:text-sm font-medium text-gray-900 border-r bg-gray-50">
                    Tiết {period} <span className="hidden md:inline">{period <= 5 ? '(Sáng)' : '(Chiều)'}</span>
                  </td>
                  {days.map(day => {
                    const session = period <= 5 ? 'Sáng' : 'Chiều';
                    const adjustedPeriod = period <= 5 ? period : period - 5;
                    ...
                    return (
                      <td key={`${day}-${period}`} 
                          className={`px-1 py-1 md:px-2 md:py-2 border-r transition-colors ${slot ? 'bg-indigo-50/50 hover:bg-indigo-100 cursor-pointer active:bg-indigo-200' : 'bg-gray-50/50'}`}
                          onClick={() => {
                            if (slot) {
                              setActiveSlot({
                          ...
                          <div className="flex flex-col items-center text-center">
                            ...
                            {assignment && (
                              <span className="mt-1 px-1 py-0.5 bg-green-100 text-green-800 text-[10px] md:text-xs rounded font-medium border border-green-200 shadow-sm whitespace-nowrap overflow-hidden text-ellipsis max-w-[50px] md:max-w-full">
                                {assignment.substituteTeacher.split(' ').pop()}
                              </span>
                            )}
                          </div>
                        ) : null}
      ...
      {activeSlot && (
        <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-0 md:p-4">
          <div className="bg-white md:rounded-2xl shadow-2xl w-full max-w-6xl h-full md:h-auto md:max-h-[90vh] flex flex-col overflow-hidden">
            <div className="p-4 md:p-5 border-b border-gray-100 bg-gray-50 flex justify-between items-start md:items-center shrink-0">
              <div className="pr-4">
              ...
              <button 
                ...
                className="text-gray-400 hover:text-gray-700 bg-gray-200 hover:bg-gray-300 rounded-full p-2 md:p-1.5 transition-colors shrink-0"
              >
            ...
            <div className="flex flex-col md:flex-row flex-1 overflow-hidden relative bg-white">
              <div className={`w-full md:w-1/2 p-4 md:p-6 overflow-y-auto border-r-0 md:border-r border-gray-200 bg-white ${selectedSub ? 'hidden md:block' : 'block'}`}>
                ...
                <h4 className="font-bold text-gray-700 mb-3 text-sm md:text-base">Gợi ý giáo viên rảnh (Cùng tổ chuyên môn)</h4>
                      ...
                      <div className="mt-6 p-4 md:p-5 bg-white rounded-xl border border-indigo-200 shadow-md relative overflow-hidden">
                        ...
                        <div className="space-y-4">
                          ...
                          <button 
                            ...
                            className="w-full bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-3.5 md:py-3 rounded-lg font-bold shadow-sm transition-colors flex justify-center items-center text-sm md:text-base active:scale-[0.98]"
                          >
                            <CheckCircle className="w-5 h-5 md:w-5 md:h-5 mr-2" />
                            Xác nhận Phân công
                          </button>
                        </div>
                      </div>
